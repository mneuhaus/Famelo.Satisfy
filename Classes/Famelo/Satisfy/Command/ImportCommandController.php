...
class ImportCommandController extends \TYPO3\Flow\Cli\CommandController {
	...
	/**
	 * @Flow\Inject
	 * @var \Famelo\Satisfy\Domain\Repository\CategoryRepository
	 */
	...
	public function fakeDataCommand() {
		// $this->emptyTable('famelo_satisfy_domain_model_user');
		// $this->emptyTable('famelo_satisfy_domain_model_customer');
		// $this->emptyTable('famelo_satisfy_domain_model_rating');
		// $this->emptyTable('famelo_satisfy_domain_model_branch_questions_join');
		// $this->emptyTable('famelo_satisfy_domain_model_branch');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Branch');

		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Rating');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Answer');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Survey');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Question');
		// $this->removeAllEntities('\TYPO3\Flow\Security\Account');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\User');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Customer');
		// $this->removeAllEntities('\Famelo\Satisfy\Domain\Model\Contact');
		...
		foreach ($questionsFixtures as $body => $weight) {
			$question = new \Famelo\Satisfy\Domain\Model\Question();
		...
		$users = array(
			...
			'Niederlassungsleiter' => array(
				...
				'lastName' => 'Tester',
				'email' => 'marc.neuhaus@neuland-medien.de',
				'role' => 'Famelo.Satisfy:Niederlassungsleiter',
				'branch' => 'Rhein/Ruhr'
			)
		);
		...
		foreach ($users as $userFixture) {
			...
			$user = new \Famelo\Satisfy\Domain\Model\User();
		...
		for ($i = 0; $i < 8; $i++) {
			$customer = new \Famelo\Satisfy\Domain\Model\Customer();
			...
			$contact = new \Famelo\Satisfy\Domain\Model\Contact();
	...
	public function importFixturesCommand($startFresh = FALSE) {
		...
		foreach ($users as $userData) {
			...
			$mail
				->setFrom(array('mneuhaus@famelo.com' => 'Satisfy'))
				->setSubject('Zugangsdaten zum Satisfy Bewertungsportal')
				->setMessage('Famelo.Satisfy:UserCreated')
				->assign('user', $user)
	...
	public function updateFromFixturesCommand($fixture) {
		...
		foreach ($fixtures as $fixture) {
			...
			if (isset($customer)) {
				...
				if (!$contact instanceof \Famelo\Satisfy\Domain\Model\Contact) {
					$contact = new \Famelo\Satisfy\Domain\Model\Contact();
					$customer->setContact($contact);
				}
